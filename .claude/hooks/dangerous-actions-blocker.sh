...
if [[ "$TOOL_NAME" == "Bash" ]]; then
    ...
    if echo "$SCAN" | grep -qE 'rm\s+(-[a-zA-Z]+\s+)*-?[a-zA-Z]*[rR][a-zA-Z]*\s+[^;&|]*(\.venv|notify/?\s*$|\.git(/|\s|$)|\.claude/worktrees)'; then
        block "Recursive delete of a repo-critical path (.venv, notify/, .git, .claude/worktrees)." \
    ...
    if echo "$SCAN" | grep -qE '(twine\s+upload|uv\s+publish|python\s+-m\s+twine|poetry\s+publish|npm\s+publish)'; then
        block "Direct package publication. PyPI never allows re-uploading a version, so a mistake here is permanent." \
            ...
            "/release <patch|minor|major>   # bump on dev -> PR to main -> GitHub Release"
    fi
