...
This guide explains how to run async-notify's Spec-Driven Development (SDD) workflow
...
async-notify configures three surfaces for Antigravity CLI:
...
- **Git Flow**: Long-lived branches `dev` (default feature base) and `main` (hotfixes only). Features never base on `main`; sub-features may base on a parent feature branch.
