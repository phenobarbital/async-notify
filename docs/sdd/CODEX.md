...
This guide explains how to run async-notify's Spec-Driven Development (SDD) flow
from Codex.
...
async-notify uses three Codex surfaces for this workflow:
...
```text
$sdd-brainstorm teams-adaptive-cards -- "Add adaptive card support for the Teams provider."
```

The brainstorm flow asks for:

1. Flow type: `feature` or `hotfix`.
2. Base branch: usually `dev`, or `main` for hotfixes only; sub-features may
   base on a parent feature branch.
...
For feature work, the default closeout opens a PR against `dev` or the
configured parent branch. Use `--merge` only when direct merge is
intentional.
...
not propagate the hotfix to `dev`.
