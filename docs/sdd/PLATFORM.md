# async-notify Spec-Driven Development (SDD) Platform
...
> `docs/sdd/GUIDE.md`, `CLAUDE.md` (SDD Git Flow + worktree policy).
...
- **Flow-aware branching (SDD Git Flow).** Every artifact carries
  `type` + `base_branch` frontmatter; features land on `dev`,
...
base_branch: dev     # feature → dev (default); hotfix → main (mandatory)
---
```
...
9.5. `--sync-down` (hotfix only): after the manual PR merges to `main`,
   propagate the change back into `dev`.
...
- **`scripts/sdd/reserve_ids.py`** / **`scripts/sdd/id_ledger.py`** — the
  git-native compare-and-swap allocator for `TASK-<NNN>`/`FEAT-<NNN>` numbers
  (`sdd/tasks/.id_ledger.json`), plus **`scripts/sdd/check_id_collisions.py`**
  as an independent read-only backstop.
...
- **`sdd-autopilot`** — shell-level agent crew that chains
...
└──────────────────────────────────────┬───────────────────────────────────┘
        ...
        Release: merge dev into main at freeze; tag the release on main.
        /sdd-done <ID> --sync-down replays hotfixes from main back to dev.
```
...
production follows the **SDD Git Flow**: `dev → main` is a manual release
merge; tagged releases live on `main`; `/sdd-done <ID> --sync-down` replays
hotfixes from `main` back into `dev`.
...
you merge that PR, run `/sdd-done <ID> --sync-down` to propagate the fix
back into `dev`.
