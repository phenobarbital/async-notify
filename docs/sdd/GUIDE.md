...
> Guía práctica para developers de async-notify. Explica cómo trabajamos con
...
```bash
/sdd-brainstorm delivery-receipt-storage -- "queremos persistir
los acuses de entrega de los providers en redis, postgres, o disco"
```
...
```bash
/sdd-spec delivery-receipt-storage
```
...
```bash
/sdd-tojira sdd/specs/delivery-receipt-storage.spec.md
...
```bash
/sdd-task sdd/specs/delivery-receipt-storage.spec.md
```
...
/sdd-status delivery-receipt-storage    # solo una
```
...
```bash
cd .claude/worktrees/feat-147-delivery-receipt-storage
/sdd-start TASK-1019
# ó
/sdd-start delivery-receipt-storage-tests
```
...
/sdd-codereview delivery-receipt-storage-tests
```
...
```bash
# 1. Charla guiada
/sdd-proposal provider-tracing -- "necesitamos trazas distribuidas
   en los providers para debugging"

# 2. Generar spec desde el proposal
/sdd-spec provider-tracing

# (Revisar el spec, marcar status: approved)

# 3. Decomponer y crear worktree
/sdd-task sdd/specs/provider-tracing.spec.md

# 4. Trabajar dentro del worktree
cd .claude/worktrees/feat-150-provider-tracing
/sdd-start TASK-001
/sdd-start TASK-002
# ...
/sdd-done FEAT-150
