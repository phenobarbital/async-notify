...
class ProviderBase(ABC):
    ...
    async def send(
        ...
        **kwargs,
        ...
        msg = await self._prepare_(recipient=recipient, message=message, **kwargs)
        rcpt = []
        results = None
        ...
        if self.blocking is True:
            ...
            fn = partial(self.execute_notify, loop, tasks)
    ...
    def execute_notify(self, loop: asyncio.AbstractEventLoop, tasks: list[Awaitable]):
        """
        execute_notify.
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Notify: Error executing Notify: {err}") from err
            except Exception as err:
                logging.exception(err, stack_info=True)
                raise ProviderError(f"Notify: Error executing Notify: {err}") from err
        except Exception as err:
            raise ProviderError(f"Notify: Exception on Notify: {err}") from err
        ...
        if callable(self.sent):
            try:
                ...
                if "task" in kwargs:
                    del kwargs["task"]
                fn = partial(
                    self.callback_sent, recipient, message, result, evt, **kwargs
                )
    ...
    def callback_sent(
        ...
        **kwargs,
        ...
        try:
            ...
            print(ex)
            raise RuntimeError(f"Error calling Callback function: {ex}") from ex
